...
class MultiHoverAviary(MultiHoverAviary_Official):
    ...
    def _computeObs(self):
        ...
        if self.OBS_TYPE == ObservationType.KIN:
            ...
            obs_obstacles= np.zeros((self.NUM_DRONES,12+len(self.fixed_positions)*5))
            for i in range(self.NUM_DRONES):
                ...
                for j in range(len(obstacles)//3):
                    if j==0:
                        ind=j*5
                        ind_obs=j*3
                        obs_obstacles[i, 12+ind:12+ind+5] = \
                            np.append(obstacles[ind_obs:ind_obs+3],[0.5,1.2])
                    else:
                        ind = j * 5
                        ind_obs = j * 3
                        obs_obstacles[i, 12 + ind:12 + ind + 5] = \
                            np.append(obstacles[ind_obs:ind_obs+3],[0.5,2])
